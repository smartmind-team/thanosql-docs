...
!!! warning ""
    컬럼을 다룰 때 `id`는 컬럼을 식별하는 데 사용되는 고유 키라는 점에 유의하는 것이 중요합니다. 이 값이 삭제되면 컬럼도 삭제된다는 의미입니다. 컬럼을 변경하는 경우 **컬럼 `id`가 본문에 있는지 확인하세요**! 새 컬럼을 추가할 때는 컬럼이 생성된 후 할당되므로 컬럼 id는 필요하지 않습니다.


다음 예제에서는 아래 테이블 객체를 다음과 같은 사항으로 변경한다고 가정해 보겠습니다:

- user_id 열의 이름을 id로 변경.
- username 열을 NULLABLE로 변경.
- 기본키 및 외래키 모두 제거.
...
- `table_name`: 데이터를 조회할 테이블 이름
- `schema`: 해당 테이블의 스키마 (기본값: 'public')
- `timezone_offset`: 날짜/시간 형식에 적용되는 시간대 (기본값: 9)
