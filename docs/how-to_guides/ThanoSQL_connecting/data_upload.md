...
SFTP 연결을 하기 위해서는 개인키와 ID가 필요합니다. 웹페이지 상단 `내 프로파일`에 들어가면 개인키를 확인 또는 재발급 받으실 수 있습니다. SFTP 접속에 필요한 ID에는 회사 ID를 입력하세요.
...
!!! warning ""
    SFTP에 접속하기 위해서는 pem 파일의 절대 경로와 STEP 1에서 확인한 회사 ID가 필요합니다.

```bash
sftp -i [pem 파일 절대경로] [회사 ID]@engine.thanosql.ai
```
